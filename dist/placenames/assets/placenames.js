...

{
   ...
   angular.module('placenames.filters', ['placenames.groups']).directive('placenamesFilters', ['placenamesSearchService', function (placenamesSearchService) {
      return {
...
function SearchService($http, $rootScope, $timeout, configService, groupsService, mapService) {
   ...
   function createParams() {
      return createSummary().then(function (summary) {
         ...
         var qs = summary.current.map(function (item) {
            return summary.filterBy + ':"' + item.name + '"';
         });
         var qas = summary.authorities.map(function (auth) {
