...
{
   angular.module("placenames.categories", []).directive("placenamesCategories", ['groupsService', function (groupsService) {
      return {
...
{
   angular.module("placenames.download", []).directive("placenamesDownload", ["placenamesDownloadService", function (placenamesDownloadService) {
      return {
      ...
      return {
         template: '<a href"javascript:void()" ng-click="pr.download()" ng-show="pr.data.searched.data.response.docs.length" ' + 'uib-tooltip="Download listed features in ESRI JSON format" tooltip-placement="bottom"><i class="fa fa-download"></i></a>'
      };
      ...
      var service = {
         data: {
            ...
            get percentComplete() {
               return (this.validEmail ? 100 / 3 : 0) + (this.validProjection ? 100 / 3 : 0) + (this.validFormat ? 100 / 3 : 0);
            }
         },
...
{
   angular.module("placenames.feature", []).directive("placenamesFeatures", ['groupsService', function (groupsService) {
      return {
...

{
   ...
   angular.module('placenames.filters', ['placenames.groups']).directive('placenamesFilters', ['placenamesFiltersService', function (placenamesFiltersService) {
      return {
      ...
      var getAuthoritiesTable = function getAuthoritiesTable() {
      ...
      var convertToEntries = function convertToEntries(data) {
         var response = [],
             entry,
             code;
...

{
   ...
   var decorateGroups = function decorateGroups(groups) {
   ...
   angular.module("placenames.groups", ["placenames.feature", "placenames.categories"]).directive("placenamesGroups", ['groupsService', function (groupsService) {
      return {
...

{
   ...
   }]).directive("placenamesQuickSearch", ['$rootScope', '$timeout', 'groupsService', 'placenamesFiltersService', 'placenamesSearchService', function ($rootScope, $timeout, groupsService, placenamesFiltersService, placenamesSearchService) {
      return {
   ...
   }]).directive("placenamesSearch", ['$timeout', 'groupsService', 'placenamesFiltersService', 'placenamesSearchService', function ($timeout, groupsService, placenamesFiltersService, placenamesSearchService) {
      return {
         ...
         link: function link(scope) {
            ...
            scope.$watch("status.catOpen", function (load) {
               if (load) {
                  ...
                  if (!scope.categories) {
                     scope.features = true;
                     groupsFiltersService.getCategories().then(function (categories) {
            ...
            scope.$watch("status.featureOpen", function (load) {
               if (load) {
                  ...
                  if (!scope.features) {
                     scope.features = true;
                     groupsFiltersService.getFeatures().then(function (features) {
            ...
            scope.$watch("status.authOpen", function (load) {
               if (load && !scope.authorities) {
                  ...
                  placenamesFiltersService.getAuthorities().then(function (authorities) {
...
$templateCache.put("placenames/quicksearch/filteredsummary.html","<div>Matched {{state.persist.data.response.numFound | number}}</div>");
